  ...
  if (cart.length === 0) {
    return (
      <div className="page-wrapper text-center" style={{ paddingTop: '80px' }}>
        ...
        <h4 style={{ color: '#111', fontWeight: '600', marginBottom: '8px' }}>{t('cart.emptyTitle')}</h4>
        <p style={{ color: '#6b7280', marginBottom: '24px' }}>{t('cart.emptyText')}</p>
        <Link to="/"><Button variant="primary">{t('cart.backToShop')}</Button></Link>
      </div>
    )
  }
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="page-title mb-1">{t('cart.title')}</h2>
          <span style={{ color: '#6b7280', fontSize: '0.9rem' }}>
            {totalItems} {totalItems === 1 ? t('cart.item') : t('cart.items')}
          </span>
        </div>
        <Button variant="outline-danger" size="sm" onClick={clear}>{t('cart.clearCart')}</Button>
      </div>
        ...
        <div>
          <div className="section-card p-0" style={{ overflow: 'hidden' }}>
            ...
            <div className="d-none d-md-grid" style={{
              ...
              <span>{t('cart.colPrice')}</span>
              <span>{t('cart.colQuantity')}</span>
              <span>{t('cart.colTotal')}</span>
              <span></span>
            </div>
